...
class App extends Component {
  ...
  componentDidMount() {
    ...
    return http.get('/getItems').then((response) => {
      this.setState({
  ...
  addItem(event) {
    ...
    if (isOnTheList) {
      ...
      return newItem !== '' && http.post('/addItem', { item: newItem }).then((response) => {
        this.setState({
          buyItems: response.data,
          message: ''
        })
  ...
  removeItem(item){
    ...
    return http.delete(`/deleteItem?id=${item.id}`).then((response) => {
      this.setState({
        buyItems: response.data
      })
    })
